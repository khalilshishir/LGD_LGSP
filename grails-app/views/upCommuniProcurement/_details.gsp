...
//        alert(rate)
    ...
    var childCount = ${upCommuniProcurementInstance?.upCommuniProcurementDetails?.size()} + 0;
...
//debugger;
        ...
        var htmlId = 'upCommuniProcurementDetails[' + childCount + '].';
...
<div class="" style="border: solid 1px;border-radius: 5px">
    <table  style=" border-radius:5px;table-layout: fixed" id="detailList" class="table table-bordered table-striped table-hover table-condensed">
        ...
        <g:each var="upCommuniProcurementDetails" in="${upCommuniProcurementInstance.upCommuniProcurementDetails}" status="i" >
            <g:render template='detail' model="['upCommuniProcurementDetails':upCommuniProcurementDetails,'i':i]"/>
        </g:each>
    </table>
