...
//        alert(rate)
    ...
    var childCount = ${upOtmProcurementInstance?.upOtmProcurementDetails?.size()} + 0;
...
//debugger;
        ...
        var htmlId = 'upOtmProcurementDetails[' + childCount + '].';
...
<div class="" style="border: solid 1px;border-radius: 5px">
    <table  style=" border-radius:5px;table-layout: fixed" id="detailList" class="table table-bordered table-striped table-hover table-condensed">
        ...
        <g:each var="upOtmProcurementDetails" in="${upOtmProcurementInstance.upOtmProcurementDetails}" status="i" >
            <g:render template='detail' model="['upOtmProcurementDetails':upOtmProcurementDetails,'i':i]"/>
        </g:each>
    </table>
