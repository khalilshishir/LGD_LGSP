...
<%@ page import="comonclass.ProcPlanStatus; comonclass.ProcPlanType; procurement.pmu.PmuProcPlan" %>
<!DOCTYPE html>
<html>
	<head>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <a href="${g.createLink(controller: 'pmuProcPlan')}" class="close" aria-hidden="true">&times;
                        </a>
                    ...
                    <div class="modal-footer">
                        <a href="${g.createLink(controller: 'pmuProcPlan')}" aria-hidden="true" class="btn btn-default">Close</a>
        ...
        <div class="modal" id="myModal12" >
            ...
            <table id="example" class="dataListTable table table-bordered table-striped table-hover table-condensed">
                ...
                <tr>
                    ...
                    <th>Procurement Guide Line</th>
                    <th>Planed Date</th>
                    <th>Progress Status</th>
                    %{--<th><tr><th>Progress Status</th></tr><tr><th>Active</th><th>In-Active</th><th>In-Planning</th><th>In-Progress</th></tr></th>--}%
                    <th>Add WBS</th>

                </tr>
                </thead>
				<tbody>
				<g:each in="${pmuProcPlanInstanceList}" status="i" var="pmuProcPlanInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">

						<td><g:link action="show" id="${pmuProcPlanInstance.id}">${fieldValue(bean: pmuProcPlanInstance, field: "packageNo")}</g:link></td>

						<td>${pmuProcPlanInstance.contractDescription}</td>

						<td>${fieldValue(bean: pmuProcPlanInstance, field: "unit")}</td>

						<td>${fieldValue(bean: pmuProcPlanInstance, field: "quantity")}</td>

						<td>${fieldValue(bean: pmuProcPlanInstance, field: "estimatedPrice")}</td>

						<td>${fieldValue(bean: pmuProcPlanInstance, field: "procurementMethod")}</td>
						<td>${fieldValue(bean: pmuProcPlanInstance, field: "procurementGuideLine")}</td>
						<td>${fieldValue(bean: pmuProcPlanInstance, field: "planedDate")}</td>
						%{--<td>${fieldValue(bean: pmuProcPlanInstance, field: "pmuProcurPlanWbs")}</td>--}%
                       <td class="center">${pmuProcPlanInstance?.pmuProcurPlanWbs.status ? pmuProcPlanInstance?.pmuProcurPlanWbs.status :"No WBS Added Yet"}</td>

                        <td class="actions ">
                            <div class="btn-group">
                                <a class="addWbs btn btn-warning" id="addWBS" referenceId="${fieldValue(bean: pmuProcPlanInstance, field: "id")}"
    ...
    <script type="application/javascript">
        $('.addWbs').click(function () {
            ...
            $.ajax({
                type:"POST",
                url:"${createLink(controller: 'pmuProcPlan', action: 'showone')}?id="+referenceId,
                dataType: "json",
